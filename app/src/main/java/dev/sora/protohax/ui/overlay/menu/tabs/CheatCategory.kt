				...
				Column(modifier = Modifier.padding(PaddingValues(13.dp, 3.dp, 13.dp, if (expand) 13.dp else 3.dp))) {
					Box(modifier = Modifier.fillMaxWidth()) {
						...
						Switch(
							...
							onCheckedChange = {
								module.state = it
								//if(!module.state) SoundManager().Disable(overlayManager.ctx) else SoundManager().Enable(overlayManager.ctx)
...
private fun CheatShortcut(
	...
	Row(
		...
		Text(text = if(MinecraftRelay.chineseMode) "快捷键" else "Shortcut", modifier = Modifier.basicMarquee(iterations = Int.MAX_VALUE))
