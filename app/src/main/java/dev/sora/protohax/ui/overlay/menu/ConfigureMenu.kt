		...
		composeView.setContent {
			MyApplicationTheme(
				...
				val navGraph = remember {
					...
					navController.createGraph(if(MinecraftRelay.chineseMode) categories[0].chinese else categories[0].choiceName) {
						...
						categories.forEach { category ->
							composable(if(MinecraftRelay.chineseMode) category.chinese else category.choiceName) {
								Box(modifier = Modifier.fillMaxSize()) {
	...
	private fun Content(navController: NavHostController, navGraph: NavGraph, scrollState: ScrollState) {
		Card(
			...
			Row {
				...
				val selectedDestination = navBackStackEntry?.destination?.route ?: if(MinecraftRelay.chineseMode) categories[0].chinese else categories[0].choiceName
				...
				NavigationRail(
					...
					categories.forEach { item ->
						NavigationRailItem(
							...
							label = { Text(if(MinecraftRelay.chineseMode) item.chinese else item.choiceName) },
